...
class UniformPatchEmbed(nn.Module):
    ...
    def forward(self, x):
        B, C, T, H, W = x.shape  # Batch（第一次训练中输入的样本数量）, channels, frames(time), height, width
...
class VIVIT(nn.Module):
    def __init__(self, Tactile_img_size, Visual_img_size, Tactile_patch_size, Visual_patch_size, in_chans, num_classes,
                 embed_dim, depth, num_heads, mlp_ratio, qkv_bias, qk_scale, drop_rate, attn_drop_rate, drop_path_rate,
                 num_frames, dropout, use_gpu=True):
        ...
        self.patch_embed_z = UniformPatchEmbed(img_size=Visual_img_size, patch_size=Visual_patch_size,
                                               in_chans=in_chans, embed_dim=embed_dim)
        num_patches_z = self.patch_embed_z.num_patches
        self.patch_embed_w = UniformPatchEmbed(img_size=Tactile_img_size, patch_size=Tactile_patch_size,
                                               in_chans=in_chans, embed_dim=embed_dim)
        ...
        # self.pos_embed = nn.Parameter(torch.zeros(1,num_patches_z+1, embed_dim))
        ...
        self.fusion = nn.Linear(2 * embed_dim, 40) #- ----------------------------------------------44444
    ...
    # z -> visual_sliding image
    # w -> tactile_sliding image
    def forward(self, x2, y2, thresh):  # self,x1,x2,y1,y2,thresh
        if self.use_gpu == True:
            # x1 = x1.to('cuda')
            x2 = x2.to('cuda')
            # y1 = y1.to('cuda')
